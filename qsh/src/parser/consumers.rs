...
impl Consumer for HexCharacter {
	fn try_consume(input: &[char], start: usize) -> ParserResult<Self> {
		...
		let c = u32::from_str_radix(&encoded_char, 16)
			.unwrap_or_else(|_| panic!("BUG: Invalid hex character: {}", encoded_char));
...
mod tests {
	...
	fn test_hex_character_consumer() {
		...
		let token = HexCharacter::try_consume(&['\\', 'u', '1', '2', 'b', 'd'], 0)
			.unwrap()
			.unwrap();
	...
	fn test_escaped_string_char_consumer() {
		...
		let token = EscapedStringChar::<'"'>::try_consume(&['\\', 'u', '1', 'f', 'b', '8'], 0)
			.unwrap()
			.unwrap();
